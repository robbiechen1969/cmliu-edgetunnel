...
// 常量定义
const subConverter = 'SUBAPI.fxxk.dedyn.io';
...
const DLS = 8;
const remarkIndex = 1; // CSV备注所在列偏移量
const FileName = atob('ZWRnZXR1bm5lbA==');
const httpsPorts = ["2053", "2083", "2087", "2096", "8443"];
const 有效时间 = 7;
const 更新时间 = 3;
const path = '/?ed=2560';
const banHosts = [atob('c3BlZWQuY2xvdWRmbGFyZS5jb20=')];
...
async function 整理(内容) {
    ...
    if (替换后的内容.charAt(0) === ',') 替换后的内容.slice(1);
    if (替换后的内容.charAt(替换后的内容.length - 1) === ',') 替换后的内容.slice(0, 替换后的内容.length - 1);
    return 替换后的内容.split(',');
}
